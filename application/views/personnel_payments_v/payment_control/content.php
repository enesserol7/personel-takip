<div class="widget widget-07 has-shadow">
	...
	<div class="widget-header bordered d-flex align-items-center">
		...
		<div class="widget-options">
			<div class="btn-group" role="group">
				<?php if ($item) { ?>
					<a href="<?php echo base_url("personnel_payments/control_institution_all_payment_confirmation/$item->id"); ?>" class="btn btn-shadow btn btn-gradient-04" title="Personeller Ay Boyunca Eksiksiz Çalıştı ise Alttaki Formu Kullanmanıza Gerek Yoktur!! Toplu Ödeme Yapmak için Butona Tıklayınız...">Toplu Ödeme Yap</a>
				<?php }else{ ?>
					<a href="<?php echo base_url("personnel_payments/control_all_payment_confirmation"); ?>" class="btn btn-shadow btn btn-gradient-04" title="Personeller Ay Boyunca Eksiksiz Çalıştı ise Alttaki Formu Kullanmanıza Gerek Yoktur!! Toplu Ödeme Yapmak için Butona Tıklayınız...">Toplu Ödeme Yap</a>
				<?php } ?>
			</div>
		</div>
	</div>
	...
	<center><div class="col-lg-5">
		<form class="needs-validation" novalidate action="<?php echo base_url("personnel_payments/control_institution_personnel_list"); ?>" method="post">
				...
				<table id="export-table" class="table table-hover mb-0 content-container">
					<thead>
						<tr>
							...
							<!--<th>Çalıştığı Gün</th>-->
                            <th>Alacağı Ödeme</th>
                            <th>Bankadan Alacağı Ödeme</th>
                            <th>Elden Alacağı Ödeme</th>
                            <th>BES Ödeme</th>
                            <th>İcra</th>
							<th>İşlemler</th>
						</tr>
					...
					<tbody class="sortable" data-url="<?php echo base_url("personnel/rankSetter"); ?>">
						...
						<?php foreach ($itemss as $item) { ?>
							<tr id="ord-<?php echo $item->id; ?>">
								...
								<td><?php $advance_payment = floatval($item->advance_payment); echo number_format($advance_payment, 2, ',', '.'); ?></td>
						...
						<tr>
							...
							<td><?php $totalAdvance =  0;?>
							<?php foreach ($itemss as $item) { ?>
								<?php $totalAdvance += floatval($item->advance_payment); ?>
							<?php } ?>
							<span><b>Toplam Avans: </b> <b><?php echo number_format($totalAdvance, 2, ',', '.'); ?> </b></span></td>
							<!--<td></td>-->
							<td><?php $receivableAmountTotal =  0;?>
							<?php foreach ($itemss as $item) { ?>
								<?php $receivableAmountTotal += floatval($item->receivable_amount); ?>
							<?php } ?>
							<span><b>Ödenecek Toplam Maaş: </b> <b><?php echo number_format($receivableAmountTotal, 2, ',', '.'); ?> </b></span></td>
