...
class Personnel_exit extends MY_Controller {
	public $viewFolder = "";
    public function __construct(){
        parent::__construct();
        $this->viewFolder = "personnel_exit_v";
    ...
    public function index(){
        ...
        if (!isAllowedViewModule()) {
            redirect(base_url("personnel_exit"));
        }
        ...
        //}
        if ($this->session->userdata("user")) {
            $items = $this->personnel_exit_model->get_all(
                array(
                    "isActive" => 1
                ),"rank ASC"
            );
            ...
            foreach ($institutions as $institution) {
                if (isAllowedViewInstitution($institution->id)) {
                    $item1 = $this->personnel_exit_model->get_all(
                        array(
                            "institution_id" => $institution->id,
                            "isActive" => 1
                        ),"rank ASC"
    ...
    public function save(){
        ...
        if ($output_reason == "istifa") {
            if($_FILES["document"]["name"] == ""){
                $alert = array(
                    "title" => "İşlem Başarısız Oldu!",
                    "text" => "Çıkış nedeni istifa olarak seçildi istifaya ait belge yüklemelisiniz!!!",
                    "type" => "error"
                );
                $this->session->set_flashdata("alert",$alert);
                redirect(base_url("personnel_exit/new_form"));
                die();
            }
        }
        ...
        if($validate){
            ...
            if($_FILES["document"]["name"] !== ""){
                ...
                if($upload_document){
                    ...
                    $this->session->set_flashdata("alert",$alert);
                    redirect(base_url("personnel_exit/update_form/$id"));
                }
            }
            $insert = $this->personnel_exit_model->add($data);
            if($insert){
                $alert = array(
    ...
    public function update($id){
        ...
        if($validate){
            ...
            $update = $this->personnel_exit_model->update(array("id"=>$id),$data);
            if($update){
                $alert = array(
                    "title" => "İşlem Başarılıyla Gerçekleşti.",
    ...
    public function delete($id){
        ...
        $delete = $this->personnel_exit_model->delete(
          array(
             "id" => $id
         )
      );
...
public function isActiveSetter($id){
    ...
    if ($id) {
      ...
      $this->personnel_exit_model->update(
         array(
            "id" => $id
        ),
         array(
